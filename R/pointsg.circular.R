...
pointsg.circular <- function(x, pch = 16, cex = 1, stack = FALSE, start.sep=0, sep = 0.025, shrink=1, bins=NULL, col=NULL, next.points=NULL, plot.info=NULL, zero=NULL, rotation=NULL, ...) {
   ...
   for (iseries in 1:nseries) {
      ...
      if (n) {
         ...
         x <- x+zero
         x <- x%%(2*pi)
         PointsCircularRadG(x, bins, stack, col, pch, iseries, nseries, start.sep, sep, next.points, shrink, cex, ...) 
      }
   }
return(invisible(list(zero=zero, rotation=rotation, next.points=next.points+nseries*sep)))
}

PointsCircularRadG <- function(x, bins, stack, col, pch, iseries, nseries, start.sep, sep, next.points, shrink, cex, ...) {
